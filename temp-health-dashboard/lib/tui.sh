...
render_loading() {
  ...
  if command -v dialog >/dev/null 2>&1; then
    dialog --title "Running health checks" --infobox "$message" 5 70 2>/dev/tty || true
  elif command -v whiptail >/dev/null 2>&1; then
    whiptail --title "Running health checks" --infobox "$message" 5 70 2>/dev/tty || true
  else
...
maybe_prompt_for_ssh_password() {
  ...
  for host in "${!HOST_IPS[@]}"; do
    ...
    while IFS= read -r ip; do
      ...
      status=0
      if command -v dialog >/dev/null 2>&1; then
        password="$(dialog --insecure --title "SSH password: $host" --passwordbox "Enter SSH password for $host (used for all IPs), or Cancel to skip." 10 70 2>&1 >/dev/tty)" || status=$?
      elif command -v whiptail >/dev/null 2>&1; then
        password="$(whiptail --title "SSH password: $host" --passwordbox "Enter SSH password for $host (used for all IPs), or Cancel to skip." 10 70 2>&1 >/dev/tty)" || status=$?
      else
        return 0
      fi
      if [[ "$status" -eq 0 && -n "$password" ]]; then
        SSH_PASSWORDS[$host]="$password"
        render_loading "Retrying SSH for $host"
...
build_dashboard_text() {
  ...
  printf '\nTIME SYNC / NTP\n%-18s %-15s %-12s %s\n' HOST IP STATUS DETAIL
  for host in "${!HOST_IPS[@]}"; do while IFS= read -r ip; do line="$(get_timesync_status "$host" "$ip")"; state="${line#*=}"; state="${state%%|*}"; detail="${line#*|}"; printf '%-18s %-15s %-12s %s\n' "$host" "$ip" "$state" "$detail"; done < <(host_ips "$host"); done
}
